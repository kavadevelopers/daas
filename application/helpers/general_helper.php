...
function sendPushN($tokon,$title,$body,$type = '',$dy = ""){
    ...
    $serverKey = get_setting()['fserverkey'];
    $notification = array('title' => $title, 'body' => $body,'sound' => 'default','badge' => '0');
    $arrayName = array('alert' => ['title' => $title, 'body' => $body],'sound' => 'default','badge' => '0');
    $arrayToSend = array('registration_ids' => $tokon,"priority" => "high","aps" => $arrayName,'data' => ['title' => $title,'body' => $body,'type' => $type,'dy' => $dy]);
    $json = json_encode($arrayToSend);
    $headers = array();
    $headers[] = 'Content-Type: application/json';
    $headers[] = 'Authorization: key='. $serverKey;
