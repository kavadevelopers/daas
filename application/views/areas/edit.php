...
<div class="page-body">
    <div class="card">
        <form method="post" action="<?= base_url('areas/update') ?>">
            <div class="card-block">
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            <label>Area Name <span class="-req">*</span></label>
                            <input name="name" type="text" class="form-control" value="<?= set_value('name',$single['name']); ?>" placeholder="Area Name" required>
                            <?= form_error('name') ?>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="form-control">
                                <?php foreach ($service_partners as $key => $value) { ?>
                                    <div class="checkbox-fade fade-in-primary d-">
                                        <label>
                                            <input type="checkbox" name="service[]" value="<?= $value['id'] ?>" <?= in_array($value['id'], explode(',', $single['services']))?'checked':'' ?>>
...
<script type="text/javascript">
    ...
    function initialize() {
        ...
        var triangleCoords = [
            <?php foreach (explode('-', $single['latlon']) as $key => $value) { ?>
                new google.maps.LatLng('<?= explode(",", $value)[0] ?>','<?= explode(",", $value)[1] ?>'),
            <?php } ?>
        ];
