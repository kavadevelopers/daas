...
class Addchat_lib 
{
	...
	*/
	...
	public function get_buddy()
	{
		...
		->set_rules('user', $this->AC_LIB->lang->line('user'), 'trim|is_natural_no_zero');
	...
	*/
	public function send_message()
	{
		...
		/* Validate form input */
        $this->AC_LIB->form_validation
        ->set_rules('user', $this->AC_LIB->lang->line('user'), 'required|trim|is_natural_no_zero')
        ->set_rules('message', $this->AC_LIB->lang->line('message'), 'trim|required|max_length[2000]');
	...
	*/
	public function delete_chat($user_id = null)
	{
        ...
        if(empty($user_id))
        {
        	$data  =  array('status' => false, 'response'=> $this->AC_LIB->lang->line('delete').' '.$this->AC_LIB->lang->line('fail'));
			$this->format_json($data);
        }
	...
	*/
    public function profile_update()
    {
		...
		$this->AC_LIB->form_validation
		->set_rules('status', $this->AC_LIB->lang->line('status'), 'required|trim')
		->set_rules('fullname', $this->AC_LIB->lang->line('fullname'), 'required|trim')
		->set_rules('user_id', $this->AC_LIB->lang->line('user'), 'required|trim|is_natural_no_zero');
		...
		if(! empty($_FILES['image']['name'])) // if image 
        {
            ...
            {
				$data	=	array('status' => false, 'response'=> $this->AC_LIB->lang->line('image_upload').' (png | jpg | jpeg)' );
				$this->format_json($data);
	        }
        }
    ...
    */
	...
	public function message_delete($message_id = null)
	{
		...
		if($status)
			$this->format_json(array('status' => true, 'message' => $this->AC_LIB->lang->line('message').' '.$this->AC_LIB->lang->line('deleted')));
		
		$this->format_json(array('status' => false, 'message'=> $this->AC_LIB->lang->line('delete').' '.$this->AC_LIB->lang->line('fail')));
	}
	...
	*/
	public function save_settings()
	{
		...
		/* Validate form input */
		$this->AC_LIB->form_validation
		->set_rules('site_name', $this->AC_LIB->lang->line('site_name'), 'required|trim')
		->set_rules('footer_text', $this->AC_LIB->lang->line('footer_text'), 'trim')
		->set_rules('footer_url', $this->AC_LIB->lang->line('footer_text').' '.$this->AC_LIB->lang->line('URL'), 'trim')
		->set_rules('admin_user_id', $this->AC_LIB->lang->line('admin').' '.$this->AC_LIB->lang->line('user').' '.$this->AC_LIB->lang->line('id'), 'required|trim|is_natural_no_zero')
		->set_rules('pagination_limit', $this->AC_LIB->lang->line('pagination_limit'), 'required|trim|is_natural_no_zero')
		->set_rules('img_upload_path', $this->AC_LIB->lang->line('img_upload_path'), 'required|trim')
		->set_rules('assets_path', $this->AC_LIB->lang->line('assets_path'), 'required|trim')
		->set_rules('users_table', $this->AC_LIB->lang->line('users_table').' '.$this->AC_LIB->lang->line('name'), 'required|trim')
		->set_rules('users_id', $this->AC_LIB->lang->line('users_id'), 'required|trim')
		->set_rules('users_email', $this->AC_LIB->lang->line('users_email'), 'required|trim')
		->set_rules('notification_type', $this->AC_LIB->lang->line('notification_type'), 'numeric');
    ...
    */
    private function check_auth()
    {
        if(!$this->AC_SETTINGS->logged_user_id) 
    		$this->format_json(array('status' => false, 'response'=> $this->AC_LIB->lang->line('access_denied')));
